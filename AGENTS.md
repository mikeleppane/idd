...
| `forge-resync-agents`       | `skills/forge-resync-agents/SKILL.md`       | explicit | Re-sync AGENTS.md / CLAUDE.md / README.md prose into enforcement mechanisms. Reads bounded signals via `tools.constitution_amend.collect_resync_signals`, drafts a convention inventory classified by mechanism (hook / validator / reviewer-tag / advisory), routes accepted entries: validator/hook entries append to `.forge/conventions.json` via `tools.constitution_amend.append_conventions_entries`; reviewer-tag entries route the user to `/forge:amend-constitution`; advisory entries log to decisions.md. |
| `forge-lesson`              | `skills/forge-lesson/SKILL.md`              | explicit | Sequential interactive authoring for one cross-feature trap lesson. Walks the user through trap / avoidance / severity / captured-from / tag draft (from the controlled vocabulary in `tools.intel.lessons._TAG_VOCAB`) / accept-or-edit-or-cancel selector. On accept, calls `tools.intel.lessons.append`. Uses `Resolved by: manual` for non-SHA resolutions. |
...
| `forge-qa`                  | `skills/forge-qa/SKILL.md`                  | explicit | Post-merge black-box QA pass on a shipped feature. Adversarial, outsider-perspective verification against acceptance criteria + negative requirements; surfaces gaps the feature author missed. Standard / full tier only. |
| `forge-tdd`                 | `skills/forge-tdd/SKILL.md`                 | explicit | TDD-pairing audit + slice scaffolding helpers. Consumed by `forge-execute` and the `tdd_evidence` validator; documents the RED → GREEN → REFACTOR contract that the validator enforces. |
...
| `/forge:validate`  | `commands/validate.md`  | Run the structural and semantic validator. Flags: `--target <spec\|plan\|delta\|scenarios\|anchors\|spec-semantic\|plan-tasks\|verified-deps\|deviations\|tdd_evidence\|domain_glossary\|qa_shape\|research\|review-lesson-tags\|constitution\|config\|conventions\|git-conventions\|health\|lessons\|ship\|all>`, optional path, `--repo-root <path>`, `--check-registries` (off by default; live registry probes for `verified-deps` / `all`), `--commit <sha>` and `--diff-file <path>` (scope `tdd_evidence`). Honours `FORGE_REPO_ROOT` env var for non-cwd repo resolution. Exit 0 / 1 (BLOCK\|HIGH) / 2 (usage). |
| `/forge:amend-constitution` | `commands/amend-constitution.md` | Open `.forge/CONSTITUTION.md` in $EDITOR for atomic edit + semver bump + decisions.md ADR entry. Pass `--bootstrap` to defer to `forge-bootstrap-constitution` for first-time drafting (bounded signal collection, in-session drafting, accept/refine/edit/skip/cancel selector). Pass `--resync-agents` to defer to `forge-resync-agents` for routing AGENTS.md / CLAUDE.md prose conventions into the right enforcement mechanism. |
| `/forge:lesson` | `commands/lesson.md` | Manually record a cross-feature trap as a lesson in `.forge/intel/lessons.md`. Dispatches the `forge-lesson` skill. Use when `/forge:review` did not auto-harvest the trap (e.g. lesson surfaced outside a feature, or resolution wasn't a SHA). |
| `/forge:qa`        | `commands/qa.md`        | Run the post-merge QA phase against a shipped feature. Outsider black-box pass that audits acceptance criteria + negative requirements without trusting the feature author's verification. Standard / full tier only. Args: `[--feature <id>] [--against merged]`. |
...
- `hooks/check_budget.py` — enforces the FORGE subagent context-budget contract on `Agent` dispatches. The hook is permissive on the optional `articles[]` field carrying filtered Constitution articles and the optional `traps[]` field carrying filtered cross-feature lessons. After the budget block validates, the hook consults `.forge/conventions.json`: rules scoped to `dispatch_brief` with severity `BLOCK` or `HIGH` deny the dispatch with the firing rule id in the deny reason.
  - **Conventions presence vs validity:** the hook decides the dispatch on three distinct states of `.forge/conventions.json`:
    - Missing `.forge/` directory entirely — the dispatch is **allowed** (pre-seed repo, nothing to enforce).
