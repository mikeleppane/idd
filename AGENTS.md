...
**Full-tier entry path (until M3 P6.2 ships `/forge:do --full`).** A clean repo cannot reach `/forge:refine` automatically: the `templates/feature/state.json` scaffold seeds `current_phase: "spec"` and `forge-spec` keeps it there, so `/forge:refine`'s phase guard refuses to run until `current_phase == "refine"`. Two supported entry paths today: (1) hand-create a feature folder under `.forge/features/<YYYY-MM-DD-slug>/` with a `state.json` pre-seeded to `current_phase: "refine"`, `tier: "full"`, plus a complete `routing` block carrying `idea`, `final_tier`, `decided_at` (see `schemas/state.schema.json` for the exact shape); or (2) wait for `/forge:do --full` adaptive routing in M3 P6.2 to seed both. The routing block is mandatory — `tools.state.increment_refine_attempts` raises `StateError` when it is absent.
