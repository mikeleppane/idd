...
def test_append_deviation_coerces_string_path(tmp_path: Path, schemas_dir: Path) -> None:
    """A ``str`` ``path`` must append the deviation identically to the ``Path`` form.
    ...
    must append the deviation identically to the ``Path`` form.
    """
    target = tmp_path / "state.json"
    initial = {
        "feature_id": "2026-05-12-coerce-dev",
        "tier": "focused",
        "current_phase": "execute",
    ...
    result = state.append_deviation(
        ...
        resolution="generated locally and noted in decisions.md",
        logged_at="2026-05-12T11:00:00Z",
        schema_path=schema_path,
    )
...
def test_set_execute_current_slice_coerces_string_path(tmp_path: Path, schemas_dir: Path) -> None:
    """A ``str`` ``path`` must set the execute slice identically to the ``Path`` form.
    ...
    that trip a cryptic ``AttributeError`` deep inside the lock-acquisition
    chain when no boundary coercion sits at the entry. The string form
    must persist the same ``current_slice`` as the ``Path`` form.
    """
    target = tmp_path / "state.json"
    initial = {
        "feature_id": "2026-05-12-coerce-exec-slice",
        "tier": "standard",
        "current_phase": "execute",
        "phases": {
            "execute": {"status": "in_progress", "started_at": "2026-05-12T10:00:00Z"},
        },
        "skipped": [],
        "deviations": [],
        "commits": [],
    ...
    assert result["phases"]["execute"]["current_slice"] == 2


def test_complete_review_target_coerces_string_path(tmp_path: Path, schemas_dir: Path) -> None:
    """A ``str`` ``path`` must complete the review target identically to the ``Path`` form.
    ...
    that trip a cryptic ``AttributeError`` deep inside the lock-acquisition
    chain when no boundary coercion sits at the entry. The string form
    must append the target to ``targets_done`` identically to the ``Path``
    form.
    """
    target = tmp_path / "state.json"
    initial = {
        "feature_id": "2026-05-12-coerce-complete-review",
        "tier": "standard",
...
def test_set_review_target_coerces_string_path(tmp_path: Path, schemas_dir: Path) -> None:
    """A ``str`` ``path`` must set the review target identically to the ``Path`` form.
    ...
    that trip a cryptic ``AttributeError`` deep inside the lock-acquisition
    chain when no boundary coercion sits at the entry. The string form
    must persist the same ``current_target`` as the ``Path`` form.
    """
    target = tmp_path / "state.json"
    initial = {
        "feature_id": "2026-05-12-coerce-set-review",
        "tier": "standard",
        "current_phase": "review",
        "phases": {
            "review": {"status": "in_progress", "started_at": "2026-05-12T10:00:00Z"},
        },
        "skipped": [],
        "deviations": [],
        "commits": [],
...
def test_guard_refine_entry_coerces_string_path(tmp_path: Path, schemas_dir: Path) -> None:
    ...
    Agent callers improvising on the call shape pass a ``str`` state.json
    path; the helper forwards directly to ``read_state(path, ...)`` whose
    first line calls ``path.exists()`` — a ``Path`` method that trips a
    cryptic ``AttributeError`` immediately when no boundary coercion sits
    at the entry. The string form must return the same parsed payload as
    the ``Path`` form for a feature legitimately sitting on refine.
    """
    target = tmp_path / "state.json"
    initial = {
        "feature_id": "2026-05-12-coerce-refine-guard",
        "tier": "full",
        "current_phase": "refine",
        "phases": {"refine": {"status": "in_progress", "started_at": "2026-05-12T10:00:00Z"}},
        "skipped": [],
    ...
    result = state.guard_refine_entry(str(target), schema_path=schema_path)
