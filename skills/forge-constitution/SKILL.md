...
description: Document the loader + relevance filter contract for `.forge/CONSTITUTION.md`. Use when reading or extending the `articles[]` dispatch budget shape, or when a phase skill needs to call `tools.constitution.load_and_filter`. Doc-only skill; not auto-invoked.
disable-model-invocation: true
---

# FORGE Constitution Loader

## When this skill applies

This skill is documentation. Phase skills (`forge-spec`, `forge-scenarios`,
`forge-plan`, `forge-crucible`, `forge-execute`, `forge-review`, `forge-verify`)
...
(`.forge/intel/`). M4 will also tighten the relevance cap with a
