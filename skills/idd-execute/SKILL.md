...
4. **Final self-review gate (both branches):**
   ...
   - Run `python -m tools.validate --target deviations .idd/features/<id>` to confirm every `state.json` deviation has a matching `decisions.md` entry. Any finding with severity `BLOCK` or `HIGH` blocks phase exit. `MEDIUM`, `LOW`, and `INFO` are advisory; surface to the user.
