...
7. If `--run`: dispatch the command via the user's slash-command runner. The skill MUST NOT mutate state directly. **Always propagate `--feature <resolved-id>`** to the dispatched command (e.g. `/idd:execute --feature 2026-05-04-demo`) so the downstream resolver acts on the same feature `/idd:next` printed — required when more than one feature exists in `.idd/features/`. If the next command already contains `--feature`, leave it as-is; otherwise append the flag verbatim.
