...
6. If `--verbose`: print a phase history table after the summary line. Pin the rendering as follows:
   ...
   - **Skipped phases:** entries from `state.json.skipped[]` render as a row with `Status` = `skipped` and the `reason` text appended in parentheses: `skipped (research deferred; manual research acceptable)`. Started/Completed columns render `none` for skipped rows.
