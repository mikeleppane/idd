---
name: idd-crucible
description: Run the three-step crucible ritual — assumptions inversion, adversarial Q&A, pre-mortem — producing UNDERSTANDING.md. Writes refutations back to SPEC.md or PLAN.md inline. Use after /idd:plan completes for standard or full tier.
...
- `.idd/features/<id>/PLAN.md` (full).
- `.idd/features/<id>/decisions.md` (append-only log).
...
`.idd/features/<id>/UNDERSTANDING.md` exists with all three ritual sections filled and a Shared Model Statement. `state.json` reflects crucible=done, current_phase=review.
