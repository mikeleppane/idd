...
4. **Mutate state BEFORE the file moves**, while `path` still points at the live feature folder. Call `tools.state.complete_phase(path, "ship")`, then `tools.state.finish_feature(path)`. The state.json now reflects `current_phase: done` at its current location and will be moved as part of the archive in step 5.
5. **Atomic ship.** Call `tools.archive.ship_feature(repo_root, feature_id, capability, body)`. This single helper:
   ...
   - rolls back the canonical-spec write if the archive move fails.
   - On any preflight failure, raises `ArchiveError` with the M2 limitation message and leaves the repo untouched. The skill should append the failure to `decisions.md` § Open and halt.
6. **Self-review gate:**
...
7. **Surface to user:** canonical spec path, archive path, capability slug, summary of what shipped (criteria count, scenarios count, evidence link).
